...
class AccessCheckoutCVVOnly_CardViewDelegateImplementation_Tests : XCTestCase {
    ...
    func testCanUpdateCvvWhenNoCvvValidator() {
        let cvvView = CVVView()
        let checkoutCvvOnly = AccessCheckoutCVVOnly(cvvView: cvvView, cvvOnlyDelegate: nil, cvvValidator: nil)
        let textForUpdate = "A"
    ...
    func testDidUpdateCvvDoesNotNotifyDelegateWhenNoCvvValidator() {
        let cvvView = CVVView()
        let cvvOnlyDelegate = MockCVVOnlyDelegate()
        let checkoutCvvOnly = AccessCheckoutCVVOnly(cvvView: cvvView, cvvOnlyDelegate: cvvOnlyDelegate, cvvValidator:nil)
        
        checkoutCvvOnly.didUpdate(cvv: "123")
    ...
    func testDidEndUpdateCvvDoesNotNotifyDelegateWhenNoCvvValidator() {
        let cvvView = CVVView()
        let cvvOnlyDelegate = MockCVVOnlyDelegate()
        when(cvvOnlyDelegate.getStubbingProxy().handleValidationResult(cvvView: any(), isValid: any())).thenDoNothing()
        let checkoutCvvOnly = AccessCheckoutCVVOnly(cvvView: cvvView, cvvOnlyDelegate: cvvOnlyDelegate, cvvValidator:nil)
        
        checkoutCvvOnly.didEndUpdate(cvv: "123")
