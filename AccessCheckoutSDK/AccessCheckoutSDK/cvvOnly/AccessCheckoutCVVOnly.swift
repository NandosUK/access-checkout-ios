...
extension AccessCheckoutCVVOnly : CardViewDelegate {
    ...
    */
    public func canUpdate(cvv: CVV?, withText text: String, inRange range: NSRange) -> Bool {
        let textChanged = applyTextChange(toText: cvv, usingRange: range, changeWithText: text)
        return cvvValidator?.validate(cvv: textChanged).partial ?? true
    }
    ...
    */
    ...
    private func applyTextChange(toText originalText:String?, usingRange range:NSRange, changeWithText:String) -> String {
        if let originalText = originalText, let range = Range(range, in: originalText) {
            return originalText.replacingCharacters(in: range, with: changeWithText)
        } else {
            return changeWithText
        }
    }
